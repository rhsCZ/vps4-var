...
function dns_get_record_from(string $server, string $type, string $record, string $protocol = 'udp'): array
{
	...
	if($protocol == "tcp")
	{
		$host = $protocol . '://' . $server . ":53";
		$socket = stream_socket_client($host,$error,$errormsg,5);
		if(!$socket = stream_socket_client($host,$error,$errormsg,5))
		{
			return ['false'];
		}
		/*$socket = socket_create(AF_INET, SOCK_STREAM, SOL_TCP);
		//socket_set_nonblock($soc);
		if (!$soc = socket_connect($socket,$server,53)) {
	...
	if($protocol == "tcp")
	{
		...
		while(!feof($socket))
		{
			//$rawbuffer = fread($socket, 1024);
			$rawbuffer = fgets($socket, 4096);
		}
		fclose($socket);
		/*$request_size = socket_write($socket,$header,$headersize);
		$error = socket_last_error();
		$errormsg = socket_strerror($error);
		$rawbuffer = socket_read($socket,4096);
		$error = socket_last_error();
		$errormsg = socket_strerror($error);
		socket_close($socket);
		*/
